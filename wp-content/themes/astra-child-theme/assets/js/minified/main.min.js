...
async function module$contents$live_map_rest_api_trajectory(){try{const c=new Headers;c.append("Source-Address","MAC-60:60:1F:5A:48:07");c.append("Limit",100);var a=Date.now();let d=(new Date(a)).toISOString();d=d.replace("T"," ").replace("Z","").substring(0,23);c.append("Latest-Timestamp",d);a=200;for(lat_lon_list=[];200==a;){const b=new Request("https://cursedindustries.com/wp-json/drones/v1/data",{method:"GET",headers:c}),e=await fetch(b),f=await e.json();a=e.status;switch(a){case 200:last_data=
f.data[f.data.length-1];d=last_data.timestamp;c.set("Latest-Timestamp",d);current_heading=last_data.heading??"Unknown";current_speed=last_data.gnd_speed??"Unknown";unique_id=last_data.unique_id??"Unknown";for(let g=0;g<f.data.length;g++)lat_lon_list.push([f.data[g].lat,f.data[g].lon]);break;case 400:console.log("Bad Request:",b);break;case 404:console.log("End of data");break;case 500:console.log("Internal server error")}404!==a&&f.error&&console.error(f.error)}drone_stats={unique_id,speed:current_speed,
heading:current_heading};console.log("Lat Lon List:",lat_lon_list);console.log("Drone stats:",drone_stats);create_trajectory(module$contents$live_map_live_map,lat_lon_list,orange_color,module$contents$live_map_orange_icon,drone_stats)}catch(c){console.error(c)}}module$contents$live_map_rest_api_trajectory();
