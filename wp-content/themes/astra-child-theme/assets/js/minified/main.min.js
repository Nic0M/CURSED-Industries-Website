...
f.setAttribute("class","table table-striped");var g=f.createTHead().insertRow();g.insertCell().innerHTML="Unique ID";g.insertCell().innerHTML="Duration";g.insertCell().innerHTML="Start Time";g.insertCell().innerHTML="End Time";g.insertCell().innerHTML="Max Height AGL";g.insertCell().innerHTML="Max Speed";var l=f.createTBody();k.flights.forEach(m=>{const n=l.insertRow();unique_id_cell=n.insertCell();unique_id_inner_hmtl=document.createElement("a");unique_id_inner_hmtl.setAttribute("href","https://google.com/search?q="+
m.src_addr);unique_id_inner_html.innerText=m.unique_id;unique_id_cell.innerHTML=unique_id_inner_html;n.insertCell().innerHTML=module$contents$database_tables_format_duration(m.duration);n.insertCell().innerHTML=m.start_time;n.insertCell().innerHTML=m.end_time;n.insertCell().innerHTML=module$contents$database_tables_format_altitude(m.max_height_agl);n.insertCell().innerHTML=module$contents$database_tables_format_speed(m.max_gnd_speed)});a.appendChild(f)}}catch(h){console.log("Error:",h)}}
let module$contents$database_tables_current_offset=0;const module$contents$database_tables_limit=25;
...
async function module$contents$live_map_rest_api_trajectory(){try{var a=new Headers;a.append("Content-Type","application/json");a.append("Source-Address","MAC-60:60:1F:DF:3F:C1");a.append("Flight-Number",1);var c=new Request("https://cursedindustries.com/wp-json/drones/v1/get_flight",{method:"GET",headers:a});const l=await (await fetch(c)).json();if(l.error)console.error(l.error);else{var d=l.unique_id??"Unknown",b=l.speed??"Unknown",e=l.heading??"Unknown",f=l.lon??[],g=l.lat??[];a=[];for(c=0;c<f.length;c++)try{a.push([g[c],
f[c]])}catch(h){console.error(h);break}drone_stats={unique_id:d,speed:b,heading:e};create_trajectory(module$contents$live_map_live_map,a,orange_color,module$contents$live_map_orange_icon,drone_stats)}}catch(l){console.error(l)}}module$contents$live_map_rest_api_trajectory();
