...
async function module$contents$live_map_rest_api_trajectory(){try{var a=new Headers;a.append("Content-Type","application/json");a.append("Source-Address","MAC-2C:CF:67:14:21:0A");a.append("Flight-Number",1);var c=new Request("https://cursedindustries.com/wp-json/drones/v1/get_flight",{method:"GET",headers:a});const l=await (await fetch(c)).json();if(l.error)console.error(l.error);else{var d=l.unique_id??"Unknown",b=l.speed??"Unknown",e=l.heading??"Unknown",f=l.lon??[],g=l.lat??[];a=[];for(c=0;c<f.length;c++)try{a.push([g[c],
f[c]])}catch(h){console.error(h);break}drone_stats={unique_id:d,speed:b,heading:e};create_trajectory(module$contents$live_map_live_map,a,orange_color,module$contents$live_map_orange_icon,drone_stats)}}catch(l){console.error(l)}};
