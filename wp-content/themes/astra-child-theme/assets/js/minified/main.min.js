...
async function module$contents$live_map_rest_api_trajectory(){try{const c=new Headers;c.append("Source-Address","MAC-60:60:1F:5A:48:07");c.append("Limit",100);var a=Date.now();let d=(new Date(a)).toISOString();d=d.replace("T"," ").replace("Z","").substring(0,23);c.append("Latest-Timestamp",d);a=200;let b=[],e=null,f=null,g=null;for(;200==a;){e=new Request("https://cursedindustries.com/wp-json/drones/v1/data",{method:"GET",headers:c});try{f=await fetch(e)}catch(l){console.error("Fetch error: ",l)}try{g=
await f.json()}catch(l){console.error("JSON error: ",l);break}a=f.status;switch(a){case 200:last_data=g.data[g.data.length-1];d=last_data.timestamp;c.set("Latest-Timestamp",d);current_heading=last_data.heading??"Unknown";current_speed=last_data.gnd_speed??"Unknown";unique_id=last_data.unique_id??"Unknown";for(let l=0;l<g.data.length;l++)b.push([g.data[l].lat,g.data[l].lon]);break;case 400:console.log("Bad Request:",e);break;case 404:console.log("End of data");break;case 500:console.log("Internal server error")}404!==
a&&g.error&&console.error(g.error)}drone_stats={unique_id,speed:current_speed,heading:current_heading};console.log("Lat Lon List:",b);console.log("Drone stats:",drone_stats);module$contents$drone_maps_create_trajectory(module$contents$live_map_live_map,b,orange_color,module$contents$live_map_orange_icon,drone_stats)}catch(c){console.error(c)}}module$contents$live_map_rest_api_trajectory();
