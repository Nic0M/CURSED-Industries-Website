...
goog.getCssName=function(a,c){if("."==String(a).charAt(0))throw Error('className passed in goog.getCssName must not start with ".". You passed: '+a);var d=function(e){return goog.cssNameMapping_[e]||e},b=function(e){e=e.split("-");for(var f=[],g=0;g<e.length;g++)f.push(d(e[g]));return f.join("-")};b=goog.cssNameMapping_?"BY_WHOLE"==goog.cssNameMappingStyle_?d:b:function(e){return e};a=c?a+"-"+b(c):b(a);return globalThis.CLOSURE_CSS_NAME_MAP_FN?globalThis.CLOSURE_CSS_NAME_MAP_FN(a):a};
...
goog.reflect.objectProperty=function(a,c){return a};goog.reflect.sinkValue=function(a){goog.reflect.sinkValue[" "](a);return a};goog.reflect.sinkValue[" "]=function(){};var module$exports$custom_table_search={};function module$contents$custom_table_search_calc_lcs(a,c){const d=a.length,b=c.length;let e=Array(d+1);for(var f=0;f<d+1;f++)e[f]=Array(b+1);for(f=0;f<d+1;f++)for(let g=0;g<b+1;g++)e[f][g]=0===f||0===g?0:a[f-1]===c[g-1]?1+e[f-1][g-1]:Math.max(e[f-1][g],e[f][g-1]);return e[d][b]}
function module$contents$custom_table_search_search_table(a,c,d,b=!1){var e=document.getElementById(a);if(e)if(a=document.getElementById(c))if(c=e.value,""===c)for(d=a.getElementsByTagName("tr"),b=1;b<d.length;b++)d[b].style.display="";else if(b||(c=c.toLowerCase()),e=a.getElementsByTagName("tr"),1>e.length)console.error("Table has no header row");else{a=e[0];for(let f=1;f<e.length;f++){const g=e[f].getElementsByTagName("td");let m=!1;for(let h=0;h<d.length;h++){if(h>=a.cells.length){console.error(`Column index ${d[h]} out of range`);
continue}let k=g[d[h]].innerText;switch(a.cells[d[h]].innerText){case "Unique ID":b||(k=k.toLowerCase());lcs=module$contents$custom_table_search_calc_lcs(c,k);3>Math.abs(lcs-c.length)?(m=!0,console.log(`Match found: ${c} and ${k}`)):console.log(`No match: ${c} and ${k}`);break;default:b||(k=k.toLowerCase()),k.includes(c)&&(console.log(`Default match found: ${c} and ${k}`),m=!0)}if(m){e[f].style.display="";break}}m||(e[f].style.display="none")}}else console.error(`No table with id ${c}`);else console.error(`No search bar with id ${a}`)}
...
async function module$contents$database_tables_getActiveFlights(){try{const g=await fetch("https://cursedindustries.com/wp-json/drones/v1/active-flights"),m=await g.json();console.log(m);const h=document.getElementById("active-flights-table");h.innerHTML="";h.setAttribute("style","overflow-x: scroll;");if(200!==g.status)h.innerHTML="Error: Could not fetch list of active flights. Please try again later.";else{var a=document.createElement("div");a.setAttribute("id","active-flights-search-bar-div");
h.appendChild(a);var c=module$contents$database_tables_createSearchBar("active-flights-search-bar-div","active-flights-table");a.appendChild(c);var d=document.createElement("p");d.setAttribute("class","active-flights-last-refreshed-text");h.appendChild(d);module$contents$database_tables_updateDateStr(m.current_time,"active-flights-last-refreshed-text");var b=document.createElement("table"),e=b.createTHead();b.setAttribute("class","table table-striped");var f=e.insertRow();f.setAttribute("style","background-color: #00aa00; color: black;");
f.insertCell().innerHTML="Unique ID";f.insertCell().innerHTML="Latitude";f.insertCell().innerHTML="Longitude";f.insertCell().innerHTML="Altitude";f.insertCell().innerHTML="Ground Speed";f.insertCell().innerHTML="Vertical Speed";f.insertCell().innerHTML="Heading";f.insertCell().innerHTML="Last Updated";m.flights.forEach(k=>{const l=b.insertRow();l.insertCell().innerHTML=k.unique_id;l.insertCell().innerHTML=module$contents$database_tables_format_latitude(k.lat);l.insertCell().innerHTML=module$contents$database_tables_format_longitude(k.lon);
l.insertCell().innerHTML=module$contents$database_tables_format_altitude(k.alt);l.insertCell().innerHTML=module$contents$database_tables_format_speed(k.gnd_speed);l.insertCell().innerHTML=module$contents$database_tables_format_speed(k.vert_speed);l.insertCell().innerHTML=module$contents$database_tables_format_heading(k.heading);l.insertCell().innerHTML=module$contents$database_tables_format_timestamp(k.timestamp)});h.appendChild(b);0!==module$contents$database_tables_active_flights_refresh_time_text_interval_id&&
clearInterval(module$contents$database_tables_active_flights_refresh_time_text_interval_id);module$contents$database_tables_active_flights_refresh_time_text_interval_id=setInterval(()=>module$contents$database_tables_updateDateStr(m.current_time,"active-flights-last-refreshed-text"),module$contents$database_tables_refresh_text_interval_time)}}catch(g){console.log("Error:",g)}}
async function module$contents$database_tables_getHistoricalFlights(){try{url="https://cursedindustries.com/wp-json/drones/v1/historical-flights";const h=await fetch(url,{method:"GET",headers:{"Content-Type":"application/json"}}),k=await h.json();if(200!==h.status)console.log("Error:",k);else{var a=document.getElementById("historical-flights-table");a.innerHTML="";a.setAttribute("style","overflow-x: scroll;");var c=document.createElement("div");c.setAttribute("id","historical-flights-search-bar-div");
...
f.setAttribute("class","table table-striped");var g=f.createTHead().insertRow();g.insertCell().innerHTML="Unique ID";g.insertCell().innerHTML="Duration";g.insertCell().innerHTML="Start Time";g.insertCell().innerHTML="End Time";g.insertCell().innerHTML="Max Height AGL";g.insertCell().innerHTML="Max Speed";var m=f.createTBody();k.flights.forEach(l=>{const n=m.insertRow();n.insertCell().innerHTML=l.unique_id;n.insertCell().innerHTML=module$contents$database_tables_format_duration(l.duration);n.insertCell().innerHTML=
l.start_time;n.insertCell().innerHTML=l.end_time;n.insertCell().innerHTML=module$contents$database_tables_format_altitude(l.max_height_agl);n.insertCell().innerHTML=module$contents$database_tables_format_speed(l.max_gnd_speed)});a.appendChild(f)}}catch(h){console.log("Error:",h)}}let module$contents$database_tables_current_offset=0;const module$contents$database_tables_limit=25;
async function module$contents$database_tables_getRemoteIDPackets(a){try{const k=await fetch(`https://cursedindustries.com/wp-json/drones/v1/remoteid-packets?offset=${module$contents$database_tables_current_offset}&limit=${module$contents$database_tables_limit}`),l=await k.json();if(200!==k.status)console.log("Error: Could not fetch list of Remote ID packets. Please try again later."),console.log("Response:",l);else{var c=document.getElementById("remoteid-packets-table");c.innerHTML="";c.setAttribute("style",
"");var d=document.createElement("div");d.setAttribute("id","remoteid-packets-search-bar-div");c.appendChild(d);var b=module$contents$database_tables_createSearchBar("remoteid-packets-search-bar-div","remoteid-packets-table");d.appendChild(b);var e=document.createElement("p");e.setAttribute("class","remoteid-packets-last-refreshed-text");c.appendChild(e);var f=NaN;void 0!==l.current_time?f=l.current_time:(console.log("Current time could not be parsed from the response. Displaying NaN as last refreshed time."),
...
var g=document.createElement("div");g.setAttribute("style","overflow-x: scroll; overflow-y: scroll; height: 75vh; position: relative;");c.appendChild(g);var m=document.createElement("table"),h=m.createTHead().insertRow();h.setAttribute("style","background-color: #00aa00; color: black; position: sticky; top: 0;");h.insertCell().innerHTML="Unique ID";h.insertCell().innerHTML="Timestamp";h.insertCell().innerHTML="Heading";h.insertCell().innerHTML="Ground Speed";h.insertCell().innerHTML="Vertical Speed";
h.insertCell().innerHTML="Latitude";h.insertCell().innerHTML="Longitude";h.insertCell().innerHTML="Altitude";l.packets.forEach(n=>{const p=m.insertRow();p.insertCell().innerHTML=n.unique_id;p.insertCell().innerHTML=module$contents$database_tables_format_timestamp(n.timestamp);p.insertCell().innerHTML=module$contents$database_tables_format_heading(n.heading);p.insertCell().innerHTML=module$contents$database_tables_format_speed(n.gnd_speed);p.insertCell().innerHTML=module$contents$database_tables_format_speed(n.vert_speed);
p.insertCell().innerHTML=module$contents$database_tables_format_latitude(n.lat);p.insertCell().innerHTML=module$contents$database_tables_format_longitude(n.lon);p.insertCell().innerHTML=module$contents$database_tables_format_altitude(n.geoAlt)});g.appendChild(m)}}catch(k){console.log("Error:",k)}}
async function module$contents$database_tables_getRaspberryPiStatus(){try{const m=await fetch("https://cursedindustries.com/wp-json/healthcheck/v1/get_healthchecks"),h=await m.json();if(200!==m.status)console.log("Error:",h);else{var a=document.getElementById("healthchecks-table");a.innerHTML="";if(void 0===h.healthchecks)console.log("No healthchecks found in response.");else{var c=h.healthchecks,d=document.createElement("table");d.setAttribute("class","healthchecks-table");d.setAttribute("style",
"overflow-x: scroll;");var b=d.createTHead().insertRow();b.insertCell().innerHTML="Receiver ID";b.insertCell().innerHTML="Status";b.insertCell().innerHTML="Last Updated";b.insertCell().innerHTML="Received Packets";var e=d.createTBody();c.forEach(k=>{const l=e.insertRow();l.insertCell().innerHTML=k.id;l.insertCell().innerHTML=k.status;l.insertCell().innerHTML=k.updated_at;l.insertCell().innerHTML=k.received_packets});var f=document.createElement("p");f.setAttribute("class","healthchecks-last-refreshed-text");
a.appendChild(f);var g=NaN;void 0!==h.current_time?g=h.current_time:(console.log("Current time could not be parsed from the response. Displaying NaN as last refreshed time."),g=NaN);module$contents$database_tables_updateDateStr(g,"healthchecks-last-refreshed-text");0!==module$contents$database_tables_healthchecks_refresh_time_text_interval_id&&clearInterval(module$contents$database_tables_healthchecks_refresh_time_text_interval_id);module$contents$database_tables_healthchecks_refresh_time_text_interval_id=
setInterval(()=>module$contents$database_tables_updateDateStr(g,"healthchecks-last-refreshed-text"),module$contents$database_tables_refresh_text_interval_time);a.appendChild(f);a.appendChild(d)}}}catch(m){console.log("Error:",m)}}
...
b;this.update();return this}})})();var module$exports$drone_maps={},module$contents$drone_maps_live_map=L.map("live-map",{center:[40.03,-105.1],zoom:14});L.tileLayer("https://tile.openstreetmap.org/{z}/{x}/{y}.png",{minZoom:1,maxZoom:19,zoomSnap:.25,attribution:'&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'}).addTo(module$contents$drone_maps_live_map);
var module$contents$drone_maps_DroneIconClass=L.Icon.extend({options:{iconSize:[100,100],iconAnchor:[50,50],popupAnchor:[0,-50],rotationOrigin:"center center"}}),module$contents$drone_maps_red_icon=new module$contents$drone_maps_DroneIconClass({iconUrl:"https://cursedindustries.com/images/drone_marker_icon_red.png"}),module$contents$drone_maps_orange_icon=new module$contents$drone_maps_DroneIconClass({iconUrl:"https://cursedindustries.com/images/drone_marker_icon_orange.png"}),module$contents$drone_maps_yellow_icon=
new module$contents$drone_maps_DroneIconClass({iconUrl:"https://cursedindustries.com/images/drone_marker_icon_yellow.png"}),module$contents$drone_maps_green_icon=new module$contents$drone_maps_DroneIconClass({iconUrl:"https://cursedindustries.com/images/drone_marker_icon_green.png"}),module$contents$drone_maps_cyan_icon=new module$contents$drone_maps_DroneIconClass({iconUrl:"https://cursedindustries.com/images/drone_marker_icon_cyan.png"}),module$contents$drone_maps_lilac_icon=new module$contents$drone_maps_DroneIconClass({iconUrl:"https://cursedindustries.com/images/drone_marker_icon_lilac.png"}),
module$contents$drone_maps_pink_icon=new module$contents$drone_maps_DroneIconClass({iconUrl:"https://cursedindustries.com/images/drone_marker_icon_pink.png"});red_marker=L.marker([40.03,-105.1],{icon:module$contents$drone_maps_red_icon}).addTo(module$contents$drone_maps_live_map).bindPopup("I'm a red drone moving at 50 m/s!");orange_marker=L.marker([40.04,-105.1],{icon:module$contents$drone_maps_orange_icon}).addTo(module$contents$drone_maps_live_map).bindPopup("I'm an orange drone!");
yellow_marker=L.marker([40.05,-105.1],{icon:module$contents$drone_maps_yellow_icon}).addTo(module$contents$drone_maps_live_map).bindPopup("I'm a yellow drone!");green_marker=L.marker([40.06,-105.1],{icon:module$contents$drone_maps_green_icon}).addTo(module$contents$drone_maps_live_map).bindPopup("I'm a green drone!");cyan_marker=L.marker([40.07,-105.1],{icon:module$contents$drone_maps_cyan_icon}).addTo(module$contents$drone_maps_live_map).bindPopup("I'm a blue drone!");
lilac_marker=L.marker([40.08,-105.1],{icon:module$contents$drone_maps_lilac_icon}).addTo(module$contents$drone_maps_live_map).bindPopup("I'm a purple drone!");purple_marker=L.marker([40.09,-105.1],{icon:module$contents$drone_maps_pink_icon}).addTo(module$contents$drone_maps_live_map).bindPopup("I'm a pink drone!");red_marker.setRotationAngle(45);orange_marker.setRotationAngle(90);yellow_marker.setRotationAngle(135);green_marker.setRotationAngle(180);cyan_marker.setRotationAngle(225);lilac_marker.setRotationAngle(270);
purple_marker.setRotationAngle(315);var module$contents$drone_maps_red_drone_path_lat_lon=[[40.022,-103.2],[40.023,-106.3],[40.027,-102.4],[40.031,-104.5],[40.035,-103.87],[40.03,-105.1]],module$contents$drone_maps_red_polyline=L.polyline(module$contents$drone_maps_red_drone_path_lat_lon,{color:"#ff3131",opacity:.8}).addTo(module$contents$drone_maps_live_map);
