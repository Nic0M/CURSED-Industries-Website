...
async function module$contents$database_tables_getActiveFlights(){try{const g=await fetch("https://cursedindustries.com/wp-json/drones/v1/active-flights"),l=await g.json();console.log(l);const h=document.getElementById("active-flights-table");h.innerHTML="";h.setAttribute("style","overflow-x: scroll;");if(200!==g.status)h.innerHTML="Error: Could not fetch list of active flights. Please try again later.";else{var a=document.createElement("div");a.setAttribute("id","active-flights-search-bar-div");
h.appendChild(a);var c=module$contents$database_tables_createSearchBar("active-flights-search-bar-div","active-flights-table");a.appendChild(c);var d=document.createElement("p");d.setAttribute("class","active-flights-last-refreshed-text");h.appendChild(d);module$contents$database_tables_updateDateStr(l.current_time,"active-flights-last-refreshed-text");var b=document.createElement("table"),e=b.createTHead();b.setAttribute("class","table table-striped");var f=e.insertRow();f.setAttribute("style","background-color: #00aa00; color: black;");
...
setInterval(()=>module$contents$database_tables_updateDateStr(g,"healthchecks-last-refreshed-text"),module$contents$database_tables_refresh_text_interval_time);a.appendChild(f);a.appendChild(d)}}}catch(l){console.log("Error:",l)}}
...
async function module$contents$live_map_rest_api_trajectory(){try{const c=new Headers;c.append("Source-Address","MAC-60:60:1F:5A:48:07");c.append("Limit",100);var a=Date.now();let d=(new Date(a)).toISOString();d=d.replace("T"," ").replace("Z","").substring(0,23);c.append("Latest-Timestamp",d);a=200;let b=[],e=null,f=null,g=null;for(;200==a;){e=new Request("https://cursedindustries.com/wp-json/drones/v1/data",{method:"GET",headers:c});try{f=await fetch(e),console.log("Response Headers:",f.headers)}catch(l){if(console.error(l),
f.headers.get("content_type").includes("text/html")){console.error("Wordpress HTML response");break}}g=await f.json();a=f.status;switch(a){case 200:last_data=g.data[g.data.length-1];d=last_data.timestamp;c.set("Latest-Timestamp",d);current_heading=last_data.heading??"Unknown";current_speed=last_data.gnd_speed??"Unknown";unique_id=last_data.unique_id??"Unknown";for(let l=0;l<g.data.length;l++)b.push([g.data[l].lat,g.data[l].lon]);break;case 400:console.log("Bad Request:",e);break;case 404:console.log("End of data");
break;case 500:console.log("Internal server error")}404!==a&&g.error&&console.error(g.error)}drone_stats={unique_id,speed:current_speed,heading:current_heading};console.log("Lat Lon List:",b);console.log("Drone stats:",drone_stats);create_trajectory(module$contents$live_map_live_map,b,orange_color,module$contents$live_map_orange_icon,drone_stats)}catch(c){console.error(c)}}module$contents$live_map_rest_api_trajectory();
