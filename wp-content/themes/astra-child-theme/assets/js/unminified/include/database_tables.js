...
function convert2urlsafe(str) {
	// Replace Colon, Comma, and Space
	return str.replace(/:/g, '%3A').replace(/,/g, '%2C').replace(/ /g, '%20');
}
...
async function getActiveFlights() {
	let table_div = document.getElementById('active-flights-table');
	if (table_div === null) {
		console.log("active-flights-table div does not exist on this page.")
		return;
	}
	try {
...
async function getRecentFlights() {
	let table_div = document.getElementById('recent-flights-table');
	if (table_div === null) {
		console.log("recent-flights-table div does not exist on this page.")
		return;
	}
	try {
		const url = 'https://cursedindustries.com/wp-json/drones/v1/recent-flights';
		...
		try {
			...
			console.log('Response could not be parsed as JSON:', error);
		}
		if (response.status !== 200) {
		...
		table_div.innerHTML = '';
		const table = document.createElement('table');
		table.setAttribute('class', 'table recent-flights-table');
		const header = table.createTHead();
		const header_row = header.insertRow();
		header_row.setAttribute('style', 'background-color: #aaaaff; color: black;');
		...
		data.flights.forEach(
			flight => {
				...
				let uas_id_cell = row.insertCell();
				let uas_id_cell_text = '';
				let start_time_cell = row.insertCell();
				let end_time_cell = row.insertCell();
				if (flight.end_time === null) {
					end_time_cell.innerHTML = 'N/A';
					uas_id_cell_text = '🔴 '.concat(flight.uas_id);
				}
				else {
					uas_id_cell_text = flight.uas_id
					end_time_cell.innerHTML = flight.end_time;
				}
				start_time_cell.innerHTML = flight.start_time;
				uas_id_cell_text = "<a href='/flight-information?flight_num=" + flight.flight_num + "'>" + uas_id_cell_text + "</a>";
				uas_id_cell.innerHTML = uas_id_cell_text;
			}
		);
		table_div.appendChild(table);
	}
...
async function getHistoricalFlights() {
	let table_div = document.getElementById('historical-flights-table');
	if (table_div === null) {
		console.log("historical-flights-table div does not exist on this page.")
		return;
	}
	try {
		// Fetch historical flights
		const url = 'https://cursedindustries.com/wp-json/drones/v1/historical-flights';
		const response = await fetch(
			url,
			{
				method: 'GET',
		...
		data.flights.forEach(flight => {
			...
			let html_str = "<a href='/flight-information?src_addr=" + convert2urlsafe(flight.src_addr) + "&start_time=" + convert2urlsafe(flight.start_time) + "'>" + flight.unique_id + "</a>";
...
const limit = 50; // Limit imposed server-side
async function getRemoteIDPackets(append_data=false) {
	try {
		...
		// Create table header cells
		headerRow.insertCell().innerHTML = 'UAS ID';
...
async function getRaspberryPiStatus() {
	let table_div = document.getElementById('heathchecks-table');
	if (table_div === null) {
		console.log("healthchecks-table div does not exist on this page.")
		return;
	}
	try {
...
console.log("Creating recent flights table");
getRecentFlights();
...
setInterval(getRecentFlights, 30e3);
