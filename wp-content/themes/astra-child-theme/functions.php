...
require_once('rest-api/drones.php');
...
add_shortcode('shortcode_table', 'load_shortcode_table');
...
/**
 * REST API endpoint to retrive data for most recent flights
 */
add_action(
	'rest_api_init',
	function() {
		register_rest_route('drones/v1', '/recent-flights/',
			array(
				'methods' => 'GET',
				'callback' => 'get_recent_flights',
				'permission_callback' => '__return_true',
			)
		);
	}
);
function get_recent_flights($data) {
	global $dronedb;

	$query = "SELECT flight_num, src_addr, uas_id, start_time, end_time, active FROM flights ORDER BY start_time DESC LIMIT 5;";
...
function astra_child_theme_enqueue_scripts() {
	wp_enqueue_style('custom-min-css', get_stylesheet_directory_uri() . '/assets/css/minified/main.min.css', array(), null, 'all');
	wp_enqueue_script('custom-min-js', get_stylesheet_directory_uri() . '/assets/js/minified/main.min.js', array(), null, true);
}

function load_shortcode_table($atts=[], $content=null, $tag='') { 
	ob_start();
	$id = $atts['id'] ?? 'shortcode-table';
	$height = $atts['height'] ?? '400px';
	?>
	<div id="<?php echo $id?>-table" style="height: <?php echo $height?>; overflow-x: scroll">Loading table...</div>
	<?php
	$output = ob_get_clean();
	return $output;
}
