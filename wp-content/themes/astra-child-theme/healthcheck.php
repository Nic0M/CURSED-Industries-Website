...
function handle_healthcheck($request) {
	...
	$query = $dronedb->prepare(
    	"INSERT INTO $table_name (id, status, received_packets) VALUES (%s, %s, %d) ON DUPLICATE KEY UPDATE status = VALUES(status), received_packets = VALUES(received_packets), updated_at = NOW();",
    	$id,
    	$status,
    	$received_packets
	);
...
function get_healthchecks($request) {
	global $dronedb;
	$table_name = 'healthchecks';
	$query = $dronedb->prepare("SELECT `id`, `status`, `received_packets`, `updated_at` FROM %s;", $table_name);
